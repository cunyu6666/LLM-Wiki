...
YouTube 演讲文字稿的 wiki 入口。**10 个 .md 均为真实文件副本**（2026-06-21 从软链迁移），源在 `raw/来自Youtube/`。

**目的**：让演讲稿在 wiki 知识图谱中可被引用、被检索、与 D20 演讲弹药库打通。

> ⚠️ **历史说明**：本目录早期使用 `ln -s` 软链 → raw 源，但 Obsidian 不解析 symlink，导致 wiki 中 10 个链接全部点不开。2026-06-21 改为 `cp` 真实文件 + `chmod 644`，详见 `~/CLAUDE.md` 错误日志「Obsidian Vault 软链接陷阱」。
...
- **真实文件**，权限 `644`（Obsidian 需要可读）
- **写操作改 wiki 副本**（不直接改 raw，raw 是只读历史档案）
- **raw 同步**：如果需要 raw 也更新，单独 `cp` 反向同步，不要双向编辑
- **新增文件**：放 wiki 副本即可，无需软链
- **Obsidian vault 内禁止使用软链**（详见根 CLAUDE.md 错误日志）
